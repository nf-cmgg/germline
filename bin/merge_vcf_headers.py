...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="A script to add the pedigree and sample metadata of an empty VCF to another VCF"
    )
    ...
    with open(vcf, "r") as open_vcf:
        with open(output, "w") as open_output:
            for line in open_vcf:
                if status == "Would you kindly check for the info header?" and re.search(info_pattern, line):
                ...
                elif status == "Would you kindly check for the end of the info header?" and not re.search(
                    info_pattern, line
                ):
                    ...
                    assert (
                        vcf_header == ped_header
                    ), f"The #CHROM header line does not match in both files:\nPED header: {ped_header[0]}\nVCF header: {vcf_header[0]}"
