...
| `no_dragstr` | Don't create DragSTR models to be used with HaplotypeCaller <details><summary>Help</summary><small>This currently is only able to run single-core per sample. Due to this, the process is very slow with only very small improvements to the analysis.</small></details>| `boolean` | False |  |  |
| `no_dragen_mode` | Don't use the HaplotypeCaller Dragen mode | `boolean` | False |  |  |
...
| `validationLenientMode` |  | `boolean` | True |  | True |
| `validationFailUnrecognisedParams` |  | `boolean` |  |  | True |
